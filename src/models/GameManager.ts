...
class GameManager {
    ...
    async initRound() {
        await this.uiManager.hudManager.createAssetDeck(
            () => this.networkManager.sendCommand("DrawCard", { "card_type": "Asset" }),
            this.uiManager.decksContainer,
            this.app.screen.width / 2 - 150, 70
        );
        await this.uiManager.hudManager.createLiabilityDeck(
        ...
        this.gameState.players.forEach(p => {
            p.character = null;
            p.reveal = this.gameState.getLocalPlayer() === p;
            p.playableAssets = 1;
    ...
    startTurnPlayerVisibilty() {
        ...
        if (player.playerID == this.gameState.myId) { // Use player.playerID for comparison
            this.uiManager.popUpManager.characterPopups.anounceCharacter(player);
